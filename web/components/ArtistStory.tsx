...
function rankStats(stats: ArtistStats): RankedStat[] {
  ...
  for (const def of STAT_REGISTRY) {
    ...
    // Only a percentile the corpus pass measured. Nothing is estimated from
    // a guessed median: an artist without percentiles gets the plain story.
    const measured = def.percentileKey ? stats.percentiles?.[def.percentileKey] : null;
    if (measured == null) continue;
    const percentile = measured;
...
function PlainStory({ artistName, stats }: { artistName: string; stats: ArtistStats }) {
  ...
  const cards = [
    {
      label: "The Output",
      value: stats.avg_words_per_song,
      suffix: "",
      unit: "words in the average song",
      copy: (
        <>
          {artistName} fills an average song with{" "}
          <em className="text-accent">{Math.round(stats.avg_words_per_song).toLocaleString()} words.</em> A
          catalogue of {stats.song_count.toLocaleString()} songs, read one page at a time.
        </>
      ),
      note: "The mean word count of the indexed songs. Longer does not necessarily mean faster.",
    },
    {
      label: "The Vocabulary",
      value: stats.avg_ttr * 100,
      suffix: "%",
      unit: "mean word variety",
      copy: (
        <>
          For every hundred words, about{" "}
          <em className="text-accent">{Math.round(stats.avg_ttr * 100)} are distinct</em> in the average
          song.
        </>
      ),
      note: "Distinct words divided by total words, per song and then averaged. Shorter songs tend to score higher.",
    },
    {
      label: "The Echo",
      value: stats.avg_repetition_ratio * 100,
      suffix: "%",
      unit: "mean repeated-line share",
      copy: (
        <>
          About <em className="text-accent">{Math.round(stats.avg_repetition_ratio * 100)} in every hundred lines</em>{" "}
          repeat an earlier line in the average song.
        </>
      ),
      note: "Extra appearances of identical lines, averaged per song. Text repetition, not chorus length.",
    },
        ...
        <Reveal key={card.label} className={`py-14 sm:py-20 max-w-4xl mx-auto ${i === 1 ? "md:text-right" : ""}`}>
          <div className={i === 0 && word ? "grid md:grid-cols-[1fr_280px] gap-10 items-center" : ""}>
            <div>
              ...
              <div className="figure text-ink" style={{ fontSize: "clamp(5rem, 17vw, 10rem)" }}>
                <Count value={card.value} suffix={card.suffix} />
              </div>
