...
import json, sys, os, glob, datetime  # noqa: E401
ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
names = {ln.split("\t")[1]: ln.split("\t")[2] for path in sorted(glob.glob(f"{ROOT}/output/review/*.tsv"))
         for ln in open(path, encoding="utf-8") if ln.count("\t") >= 2 and ln[:1].isdigit()}
TODAY = os.environ.get("REVIEWED", datetime.date.today().isoformat())
...
for g, r in data.items():
    ...
    out = {"artist": names.get(g, g), "reviewed": TODAY, "drop": {}, "keep": []}
