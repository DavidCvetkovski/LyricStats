...
def test_aggregate_upsert_and_get(temp_db):
    db.upsert_artist_aggregate(
        name="Drake", display_name="Drake", song_count=100,
        has_sections=True, stats={"song_count": 100},
    )
...
def test_aggregate_lookup_is_accent_and_case_insensitive(temp_db):
    db.upsert_artist_aggregate(
        name="Beyoncé", display_name="Beyoncé", song_count=50,
        has_sections=True, stats={},
    )
...
def test_aggregate_key_collision_prefers_bigger_catalogue(temp_db):
    db.upsert_artist_aggregate(name="Jay Z", display_name="Jay Z",
                               song_count=10, has_sections=False, stats={})
    db.upsert_artist_aggregate(name="JAY-Z", display_name="JAY-Z",
...
def test_aggregate_upsert_updates_existing(temp_db):
    db.upsert_artist_aggregate(name="X", display_name="X", song_count=1,
                               has_sections=False, stats={})
    db.upsert_artist_aggregate(name="X", display_name="X", song_count=99,
                               has_sections=True, stats={})
...
def test_reset_aggregates_clears_table(temp_db):
    db.upsert_artist_aggregate(name="X", display_name="X", song_count=20,
                               has_sections=False, stats={})
    db.reset_aggregates()
...
def test_suggest_finds_close_typo(temp_db):
    ...
    db.upsert_artist_aggregate(name="Taylor Swift", display_name="Taylor Swift",
                               song_count=200, has_sections=True, stats={})
